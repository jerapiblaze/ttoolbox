...
if ($Help) {
    Get-Help move-wsldisk.ps1 -Detailed
    return;
}
...
# prompt and get the distro to move if not specified
if ($Distro -eq '') {
...
# check if the target folder is a valid path and writable
try {
    $fullPath = [System.IO.Path]::GetFullPath($TargetFolder);
    if (-not (Test-Path $fullPath)) {
        throw "Target folder path does not exist: $fullPath";
    }
}
catch {
    Write-Error "Invalid target folder path: $TargetFolder";
    Exit 1;
}
...
if ($DryRun) {
    Write-Host "Dry run mode enabled. No actual move will be performed." -ForegroundColor Yellow
    Write-Host "Distro to move: $Distro" -ForegroundColor Yellow
